...
def MD_distance_similar(support_proto, support_feature, support_labels, query_features):
    NUM_SAMPLES=1
    ...
    # print('class_precision_matrices----', class_precision_matrices.shape)
    ...
    return sample_logits, class_precision_matrices

def MD_distance3(support_feature, query_features, class_precision_matrices):
    NUM_SAMPLES=1
    # class_representations, class_precision_matrices = build_class_reps_and_covariance_estimates(support_feature, support_labels)

    class_means = support_feature
    # class_precision_matrices = torch.stack(list(class_precision_matrices.values()))
    # class_means = np.array(support_feature).cpu().reshape(1, support_feature.size(0))
    # grabbing the number of classes and query examples for easier use later in the function
    # number_of_classes = class_means.size(0) 
    number_of_classes = 1
    number_of_targets = query_features.size(0)

    repeated_target = query_features.repeat(1, number_of_classes).view(-1, class_means.size(0)).cuda()
    repeated_class_means = class_means.repeat(number_of_targets, 1).cuda()
    repeated_difference = (repeated_class_means - repeated_target)
    repeated_difference = repeated_difference.view(number_of_targets, number_of_classes,
                                                   repeated_difference.size(1)).permute(1, 0, 2)
    first_half = torch.matmul(repeated_difference, class_precision_matrices)
...
def build_class_reps_and_covariance_estimates(context_features, context_labels):
    ...
    for c in torch.unique(context_labels):
        ...
        # print('flat_class_mask_indices---', flat_class_mask_indices)
        # print('context_features.size(0)--', context_features.size(0))
...
#     number_of_classes = 1
#     number_of_targets = query_features.size(0)
#     print('number_of_targets---', number_of_targets) # 80

#     repeated_target = query_features.repeat(1, number_of_classes).view(-1, class_means.size(0)).cuda()
#     print('repeated_target---', repeated_target.shape) #torch.Size([80, 128])
#     repeated_class_means = class_means.repeat(number_of_targets, 1).cuda()
#     repeated_difference = (repeated_class_means - repeated_target)
#     repeated_difference = repeated_difference.view(number_of_targets, number_of_classes,
#                                                    repeated_difference.size(1)).permute(1, 0, 2)
