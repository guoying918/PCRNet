...
def get_train_test_loader(Data_Band_Scaler, GroundTruth, class_num, shot_num_per_class):
    ...
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=class_num * shot_num_per_class, shuffle=True, num_workers=0)
    del train_dataset

    test_dataset = utils.matcifar(imdb, train=False, d=3, medicinal=0)
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=128, shuffle=False, num_workers=0)
    # test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=class_num * per_query_num, shuffle=True, num_workers=10)
    ...
    for iSample in range(da_nTrain):  # radiation_noise，flip_augmentation
        ...
        # patch = data[Row[da_RandPerm[iSample]] - HalfWidth:  Row[da_RandPerm[iSample]] + HalfWidth + 1,
        #     Column[da_RandPerm[iSample]] - HalfWidth: Column[da_RandPerm[iSample]] + HalfWidth + 1, :]
        # patch = utils.augment_hsi_patch(patch)
        # imdb_da_train['data'][:, :, :, iSample] = patch
...
def get_target_dataset(Data_Band_Scaler, GroundTruth, class_num, shot_num_per_class, query_num =None):
