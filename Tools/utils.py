...
def load_data(dataset_name, folder='./Datasets/test_ocbs/'):
    ...
    if dataset_name == 'PaviaU':
        image_data = sio.loadmat(folder + 'PaviaU.mat')['paviaU'] 
        GroundTruth = sio.loadmat(folder + 'PaviaU_gt.mat')['paviaU_gt']
    elif dataset_name == 'Salinas':
        image_data = sio.loadmat(folder + 'Salinas_corrected.mat')['salinas_corrected']
    ...
    #     GroundTruth = sio.loadmat(folder + 'Salinas_gt.mat')['salinas_gt']
        ...
        image_data = sio.loadmat(folder + 'Indian_pines_corrected.mat')['indian_pines_corrected']
        GroundTruth = sio.loadmat(folder + 'Indian_pines_gt.mat')['indian_pines_gt']
    elif dataset_name == 'LongKou':
        image_data = sio.loadmat(folder + 'WHU_Hi_LongKou.mat')['WHU_Hi_LongKou']
        GroundTruth = sio.loadmat(folder + 'WHU_Hi_LongKou_gt.mat')['WHU_Hi_LongKou_gt']
    elif dataset_name == 'HanChuan':
        image_data = sio.loadmat(folder + 'WHU_Hi_HanChuan.mat')['WHU_Hi_HanChuan']
        GroundTruth = sio.loadmat(folder + 'WHU_Hi_HanChuan_gt.mat')['WHU_Hi_HanChuan_gt']
...
def augment_hsi_patch(patch, seed=None,
                      jitter_std=0.01, band_drop_max=5, apply_prob=0.5):
    """
    对单个高光谱图像 patch 进行多种数据增强。

    参数：
        patch: np.ndarray, shape (H, W, C)，单个 patch。
        seed: int 或 None，设置随机种子以保证增强可复现。
        jitter_std: float，光谱扰动标准差。
        band_drop_max: int，最大丢弃波段数。
        apply_prob: float，每种增强操作的应用概率。
    ...
    # Step 1: radiation noise
    patch = radiation_noise(patch)

    # Step 2: Spectral jitter（光谱扰动）
    # if np.random.rand() < apply_prob:
    #     jitter = np.random.normal(0, jitter_std, patch.shape).astype(np.float32)
    #     patch += jitter

    # Step 3: Random band dropout（波段丢弃）
    # if np.random.rand() < apply_prob:
    #     n_bands = patch.shape[2]
    #     num_drop = np.random.randint(1, min(band_drop_max + 1, n_bands))
    #     drop_bands = np.random.choice(n_bands, num_drop, replace=False)
    #     patch[:, :, drop_bands] = 0

    # Step 4: Flip augmentation（随机翻转）
    if np.random.rand() < apply_prob:
        patch = np.flip(patch, axis=0)  # vertical
    if np.random.rand() < apply_prob:
        patch = np.flip(patch, axis=1)  # horizontal

    # Step 5: Rotation（0, 90, 180, 270 度旋转）
    if np.random.rand() < apply_prob:
...
class Task(object):
    def __init__(self, data, select_data, num_classes, shot_num, query_num):
        self.data = data
        self.num_classes = num_classes
        self.support_num = shot_num
        self.query_num = query_num

        class_list = select_data
        ...
        for c in class_list:
            temp = self.data[c]
            # samples[c] = random.sample(temp, shot_num + query_num)
            selected_samples = random.sample(temp, shot_num + query_num)
            samples[c] = selected_samples
            
            # selected_indices = []
            # for sample in selected_samples:
            #     idx = np.where((temp == sample).all(axis=1))[0][0]
            #     selected_indices.append(idx)
            # print(f"Class {c} selected indices:", selected_indices)
